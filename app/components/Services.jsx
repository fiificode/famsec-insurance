...
const servicesContent = {
  ...
  items: [
    ...
    },
    {
      ...
      description:
        "Innovative technology for monitoring and caring for elderly family members,promoting their safety and independence.",
    },
    {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10">
            {servicesContent?.items.map((item,idx) => (
                <div key={item.title} className="flex space-x-10">
                    ...
                    <div>
                        {item?.title && (
                            <h3 className="text-heading font-bold text-md">
                                {item.title}
                            </h3>
                        )}
                          {item?.description && (
                            <h3 className="leading-relaxed text-sm">
                                {item.description}
                            </h3>
                        )}
                    </div>
